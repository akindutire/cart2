...
<div class="col-xs-10">
<fieldset>
<legend>View All Products </legend>
<?php
$viewProduct = array();
$viewCreatedForms->viewProduct($id=false);
$viewProduct = $viewCreatedForms->allProduct;
///print_r($viewCat);
$table = '<table class="table" width="100%"><tr><td>S/N</td><td>Product Name</td><td>Remaining Quantity</td><td>Unit Price</td>
<td>Cost Price</td><td>Edit</td></tr>';
			$sn = 1;
			for($i=0; $i< count($viewProduct); $i++)
			{			 
			$table .= '<tr><td>'.$sn.'</td><td>'.$viewProduct[$i]['product_name'].'</td><td>'.$viewProduct[$i]['quantity'].'</td><td>'.$viewProduct[$i]['unit_price'].'</td><td>'.$viewProduct[$i]['cost_price'].'</td><td>'.
			'<a href="createProduct.php?proID='.$viewProduct[$i]['product_id'].'"><i>Edit <img src="image/edit.png" /></a></i>'.'</td>
			</tr>';
			$sn++;
			}
