...
<label for="role" style="margin-top:10px;">Sales Rep Name:</label> 
<select class="form-control"  style="margin-top:10px;" id="salesrep" name="salesrep" >
<option></option>
 <?php
$user=array();
$viewCreatedForms->viewUsers();
$user = $viewCreatedForms->allUser;

for($i=0; $i<count($user);$i++)
{
	echo '<option  value="'.$user[$i]['id'].'">'.$user[$i]['username'].'</option>';
}
?>
</select>
...
$profitForToday = 0;
  ...
  $table.='<tr><td colspan="2" align="center">Total Quantity Sold = '.$qsold.'</td><td align="left" colspan="2"><strong> Total Quantity Left = '.$quantityLeft.'</strong></td><td colspan="2" align="left"><strong>Total(N) Sold =  '.$amountSold.'</strong></td>
  <td  align="left"><strong>Total Cost Price(N) =  N'.$amountLeft.'</strong></td></tr>';
